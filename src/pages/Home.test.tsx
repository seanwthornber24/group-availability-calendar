...
vi.mock('date-fns', async (importOriginal) => {
  ...
  return {
    ...
    addMonths: vi.fn().mockImplementation((_date: Date, amount: number) => {
...
describe('Home', () => {
  ...
  it('submit calls createSession with trimmed name and creatorName', async () => {
    vi.mocked(createSession).mockResolvedValue('newid123')
    const user = userEvent.setup()
    renderHome()
    ...
    const args = vi.mocked(createSession).mock.calls[0][0]
