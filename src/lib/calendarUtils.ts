...
export function computeMyDays(
  ...
  return new Set(
    availability.filter((a) => a.participant_id === currentParticipantId).map((a) => a.date)
  )
}
 ...
 */
export function computeFreeCountByDate(
  availability: Pick<AvailabilityRow, 'date'>[]
...
export function getDayCellStatus(
  ...
  if (isPast) {
    return { bg: 'bg-gray-100 dark:bg-gray-800', textColor: 'text-gray-300 dark:text-gray-600' }
  }
  if (!totalParticipants || freeCount === 0) {
    return { bg: 'bg-gray-50 dark:bg-gray-900', textColor: 'text-gray-800 dark:text-gray-100' }
  }
  ...
  if (ratio >= 0.5) {
    return { bg: 'bg-amber-400', textColor: 'text-white' }
  }
